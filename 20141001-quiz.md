...
遊戲規則:
         ...
         2.玩家將輸入下注金額,贏則剩餘金額加上下注金額
                                              輸則剩餘金額扣掉下注金額
                                              平手則不做更動
           (利用coin=prompt(); 保存下注金額於coin中,parseInt(coin)可將字串轉換為int型態)
         3.比較加總數較大者勝利
           (利用for迴圈加總陣列的內容)
         4.直到剩餘金額小於0結束遊戲,Game Over!
提示: 1.可利用while迴圈設定條件剩餘金額大於0時,可繼續遊戲 (初始金額可設定為1000)
         2.程式碼中可加入  if (coin=="exit") break; 
            當您輸入exit時可自行結束遊戲
         3.執行畫面參考
         ![例1](http://i.imgur.com/2B7HMoz.png?1)        ![例2](http://i.imgur.com/ROMNaI1.png?1)
``` js
var player=[]
var com=[]
var money=1000;
